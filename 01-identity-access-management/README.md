...
5. [**OAuth 2.0 & OIDC deep dive**](notes/03-oauth-oidc-deep-dive.md) — the modern stack; why **OAuth ≠ login**, Authorization Code + PKCE, ID vs access tokens, JWT attacks.
   ...
   - ↳ [**Hitting the OpenID endpoints with Postman**](notes/26-postman-openid-endpoints.md) — the do-it-yourself companion: how to structure a Postman request to **each** endpoint in the discovery document (`/authorize`, `/token`, `/userinfo`, `/introspect`, `/revoke`, `/device_authorization`, `jwks_uri`, `end_session`) — method, headers, body, and the PKCE/token-saving scripts — plus the five mistakes that bite everyone.
